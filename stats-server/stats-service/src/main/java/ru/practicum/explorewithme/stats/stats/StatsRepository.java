...
public interface StatsRepository extends JpaRepository<Hit, Integer> {
    @Query("select new ru.practicum.explorewithme.dto.StatDto (hit.app, hit.uri, count(hit.uri)) from Hit hit " +
    ...
    @Query("select new ru.practicum.explorewithme.dto.StatDto (hit.app, hit.uri, count(distinct hit.uri)) from Hit hit " +
            "where hit.timestamp >= :start " +
            "and hit.timestamp < :end " +
            "group by hit.app, hit.uri " +
            "order by count(hit.uri) desc")
